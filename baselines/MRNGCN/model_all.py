...
class layers(torch.nn.Module):
    ...
    def __init__(self, inputdims):
        ...
        self.conv1 = graph_inception(1, inputdims, 256)#(1, 64, 256)
...
class attention(torch.nn.Module):
    ...
    def forward(self, feat):
        ...
        # print("mm", torch.mm(q, torch.transpose(k, 0, 1))/torch.sqrt(torch.tensor(self.outputdims, dtype=torch.float)))
        # print("softmax", torch.softmax(torch.mm(q, torch.transpose(k, 0, 1))/torch.sqrt(torch.tensor(self.outputdims, dtype=torch.float)), dim=1))
...
class Net(torch.nn.Module):
    def __init__(self,l_feat, r_feat, network1, network2, hop, inputdims, hiddendims, outputdims, edge_index, edge_index1):
        ...
        self.gcn1 = layers(inputdims)#layers()
        self.gcn2 = layers(inputdims)#layers()
        self.gcn3 = layers(inputdims)#layers()
        self.ba = BA(inputdims, outputdims)
        self.att = attention(outputdims, outputdims)
        self.cnn1 = CNN(outputdims)#CNN()
        self.cnn2 = CNN(outputdims)#CNN()
        self.cnn3 = CNN(outputdims)#CNN()
        self.cnn4 = CNN1()
        self.mlp = MLP(inputdims)
        self.lin1 = Linear(32, inputdims)#64)
